...
task ampseq_pipeline {
	...
	if [ "$first_line" = "sample_id,Forward,Reverse" ]; then
		echo "Sequencing run with inline barcodes. Performing analysis of combinatorial indixes followed by denoising"
		find . -type f	
		...
		echo "Sequencing run withou inline barcodes. Skipping analysis of combinatorial indixes and performing only denoising"
	fi
